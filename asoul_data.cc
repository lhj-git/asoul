...
int Asoul_data::create_table(char *path)
{
    DBUG_ENTER("AsoulIndex::create_table");
    open_table(path);
    number_records = 0;
...
int Asoul_data::open_table(char *path)
{
    DBUG_ENTER("Asoul_data::open_table");
    /*
...
long long Asoul_data::write_row(uchar *buf, int length)
{
    long long pos;
    int i;
    int len;
    uchar deleted = 0;
    DBUG_ENTER("Asoul_data::write_row");
    /*
...
long long Asoul_data::update_row(uchar *old_rec, uchar *new_rec,
                                   int length, long long position)
{
    ...
    uchar deleted = 0;
    int i = -1;
    DBUG_ENTER("Asoul_data::update_row");
    if (position == 0)
...
int Asoul_data::delete_row(uchar *old_rec, int length,
                             long long position)
{
    int i = -1;
    long long pos;
    ...
    uchar *cmp_rec;
    uchar deleted = 1;
    DBUG_ENTER("Asoul_data::delete_row");
    if (position == 0)
        position = header_size; // move past header
    pos = position;
    /*
...
int Asoul_data::read_row(uchar *buf, int length, long long position)
{
    int i;
    int rec_len;
    long long pos;
    uchar deleted = 2;
    DBUG_ENTER("Asoul_data::read_row");
    if (position <= 0)
        position = header_size; // move past header
...
/* close file */
int Asoul_data::close_table()
{
    DBUG_ENTER("Asoul_data::close_table");
    if (data_file != -1)
    {
...
int Asoul_data::records()
{
    DBUG_ENTER("Asoul_data::num_records");
    DBUG_RETURN(number_records);
}
/* return number of deleted records */
int Asoul_data::del_records()
{
    DBUG_ENTER("Asoul_data::num_records");
    DBUG_RETURN(number_del_records);
}
/* read header from file */
int Asoul_data::read_header()
{
    int i;
    int len;
    DBUG_ENTER("Asoul_data::read_header");
    if (number_records == -1)
    {
...
int Asoul_data::write_header()
{   
    int i;
    DBUG_ENTER("Asoul_data::write_header");
    if (number_records != -1)
    {   
        my_seek(data_file, 0l, MY_SEEK_SET, MYF(0));
...
long long Asoul_data::cur_position()
{
    long long pos;
    DBUG_ENTER("Asoul_data::cur_position");
...
int Asoul_data::trunc_table()
{
    DBUG_ENTER("Asoul_data::trunc_table");
    if (data_file != -1)
    {
...
int Asoul_data::row_size(int length)
{
    DBUG_ENTER("Asoul_data::row_size");
